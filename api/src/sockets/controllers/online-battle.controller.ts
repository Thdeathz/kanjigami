...
const handleBattleJoin = async (
  socket: Socket,
  { battleSlug, roundIndex }: { battleSlug: string; roundIndex: string },
) => {
...
const handleBattleRoundJoin = async (socket: Socket, { battleSlug, roundIndex, user }: IJoinRoundRequest) => {
  const battle = await redisService.get<IStartEventData>('event', battleSlug)
...
const handleBattleLeaderboard = async (socket: Socket, { battleSlug }: { battleSlug: string }) => {
...
export default {
  handleBattleJoin,
  handleBattleRoundJoin,
  handleBattleLeaderboard,
}
